...
const Login = () => {
  ...
  const loginSchema = yup.object().shape({
    username: yup
      .string()
      .required("Required")
      .min(8, "Username should be 8 characters minimum")
  ...
  const login = (username, password) => {
    axios
      .post(`${process.env.API_HOSTNAME}/auth/login`, {
        username,
        password,
      })
      .then((response) => {
        ...
        >
          {({ errors, touched }) => (
            <Form className="flex flex-col">
              ...
              <Field
                className="w-96 bg-gray-800 border-2 rounded-lg border-gray-600 p-2 transition outline-none focus:border-blue-500 focus:border-2"
                name="username"
              ...
              <button
                className="bg-gray-700 mt-2 pt-2 pb-2 rounded-md transition hover:bg-gray-600"
                type="submit"
              >
                Submit
              </button>
              ...
              <p className="mt-1 text-red-500 text-xl self-center font-bold">
                {serverError}
              </p>
            </Form>
          )}
