...
const FilePage = () => {
  ...
  const changePrivacy = () => {
    axios({
      ...
      }/files/changePrivacy/${id}?newPrivacyOption=${!file.private}`,
      validateStatus: () => true,
      headers: {
        Authorization: "Bearer " + localStorage.getItem("token"),
      },
  ...
  return (
    <div>
      ...
      <div className="bg-gray-700 h-screen flex flex-col items-center text-white">
        {serverError && (
          <p className="text-red-500 font-bold text-3xl mt-4">{serverError}</p>
        )}
        {isLoading && <p className="font-bold">Loading...</p>}
        {file && (
          <>
            <Image
              src="/white-file-icon.png"
              height={200}
              width={150}
              alt="white-file"
            />
            ...
            {file.private ? (
              <p className="rounded-xl bg-red-500 bg-opacity-75 px-2 py-1">
                Private
              </p>
            ) : (
              <p className="rounded-xl bg-green-500 bg-opacity-75 px-2 py-1">
                Public
              </p>
            )}
